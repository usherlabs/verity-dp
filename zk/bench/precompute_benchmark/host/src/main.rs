...
use verity_verify_private_transcript::{
    encodings_precompute::generate,
    presentation::{EncodingPayload, Presentation},
...
fn main() {
    ...
    let presentation_string_input = fs::read_to_string("fixtures/32b.presentation.json").unwrap();
    let presentation: Presentation = serde_json::from_str(&presentation_string_input).unwrap();

    let encodings_bytes_input: Vec<u8> = fs::read("fixtures/32b.precompute.tlsn").unwrap();
    ...
    // let encodings_precompute: HashMap<(Direction, Idx), Vec<EncodedValue<Full>>> =
    //     generate(&partial_openings, &commitment_seed).unwrap();
    // fs::write(
    //     "fixtures/32b.precompute.tlsn",
