...
use verity_verify_remote::{
    config::Config,
    ic::{Verifier, VerifyReceiptReply, DEFAULT_IC_GATEWAY_LOCAL},
};
...
fn verify_by_zkvm(
    ...
    let prove_info = prover.prove_with_opts(
        env,
        VERITY_DP_EXAMPLES_ZKTLS_GUEST_ELF,
        &ProverOpts::succinct(),
    )?;
    ...
    receipt.verify(VERITY_DP_EXAMPLES_ZKTLS_GUEST_ID)?;

    Ok((data, proof, receipt))
}

async fn verify_by_ic(receipt: Receipt) -> Result<VerifyReceiptReply, Box<dyn std::error::Error>> {
    // Perform the partial remote verification against decentralized compute

    // 1. Create a config by specifying the params
    ...
    remote_verifier.verify_receipt(receipt_bytes).await
}
