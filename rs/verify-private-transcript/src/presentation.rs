...
impl Presentation {
    ...
    /// Returns the encoding proof if it exists
    pub fn get_encoding_proof(&self) -> EncodingProof {
        return self
            .transcript
            .clone()
            .expect("TRANSCRIPT NOT FOUND")
            .encoding_proof
            .expect("ENCODING PROOF NOT FOUND");
    }

    /// getter for transcript
    pub fn get_transcript(&self) -> Option<TranscriptProof> {
        self.transcript.clone()
    }

    /// getter for attestation
    pub fn get_attestation(&self) -> AttestationProof{
    ...
    pub fn get_encodings_compute_payload(&self) -> EncodingPayload {
        ...
        let encoding_commitment = attestation_body.body.encoding_commitment().unwrap().clone();
        let seed = encoding_commitment.seed;
