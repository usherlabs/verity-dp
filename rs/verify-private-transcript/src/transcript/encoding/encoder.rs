...
use crate::transcript::{Direction, Idx, Subsequence, RX_TRANSCRIPT_ID, TX_TRANSCRIPT_ID};

pub fn new_encoder(seed: [u8; 32]) -> impl Encoder {
...
pub trait Encoder {
    ...
    fn generate_encoded_bytes(&self, direction: Direction, seq_idx: &Idx) -> Vec<EncodedValue<Full>>;
    ...
    fn encode_subsequence_with_precompute(
        &self, direction: Direction, seq: &Subsequence, precomputed_encodings: Vec<EncodedValue<Full>>
    )-> Vec<u8>;
}
...
impl Encoder for ChaChaEncoder {
    ...
    fn encode_subsequence_with_precompute(
        &self, _direction: Direction, seq: &Subsequence, precomputed_encodings: Vec<EncodedValue<Full>>
    )-> Vec<u8>{
        ...
        for (_byte_id, &byte) in seq.index().iter().zip(seq.data()) {
            ...
            let selected_encoding_value = encoded_values.select(byte)
            .expect("encoding is a byte encoding")
            .to_bytes();

            encoding.extend(
                selected_encoding_value
            );
