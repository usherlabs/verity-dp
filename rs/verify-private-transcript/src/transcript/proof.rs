...
impl TranscriptProof {
    ...
    /// Verifies the proof with some precomputed encodings provided.
    ///
    /// Returns a partial transcript of authenticated data.
    ///
    /// # Arguments
    ...
    /// * `provider` - The crypto provider to use for verification.
    /// * `attestation_body` - The attestation body to verify against.
    /// * `encodings_precompute` - The precomputed encodings from the partial openings.
    pub fn verify_with_provider_with_precompute(
        ...
        if let Some(proof) = self.encoding_proof {
            ...
            let encodings_precompute = encodings_precompute
                .ok_or_else(|| {
                    TranscriptProofError::new(
                        ErrorKind::Encoding,
                        "contains an encoding proof but precomputed encodings are missing",
                    )
                })?
                .to_owned();
...
impl TranscriptProofError {
    /// generate a new TranscriptProofError
